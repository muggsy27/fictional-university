...
class MyNotes {
  ...
  async deleteNote(e) {
    const thisNote = e.target.parentElement;
    try {
      const response = await fetch(universityData.root_url + '/wp-json/wp/v2/note/' + thisNote.getAttribute('data-id'), {
        headers: {
        ...
        ${results.professors.length ? '<ul class="professor-cards">' : `<p>No professors match that search.</p>`}
          ${results.professors.map(item => `
              ...
              <a class="professor-card" href="${item.permalink}">
                  <img src="${item.image}" alt="professor image" class="professor-card__image">
